...
export default function SellerDashboard() {
  ...
  const sellerProducts = listingsData?.listings || [];
  
  const stats = [
    { label: 'Total Revenue', value: `₹${statsData?.totalRevenue?.toLocaleString() || 0}`, change: '+12.5%', icon: <DollarSign className="w-5 h-5" />, color: 'emerald' },
    { label: 'Products Listed', value: sellerProducts.length.toString(), change: '+2', icon: <Package className="w-5 h-5" />, color: 'blue' },
    { label: 'Avg Rating', value: statsData?.avgRating?.toString() || '0.0', change: '+0.2', icon: <Star className="w-5 h-5" />, color: 'amber' },
    { label: 'Total Sales', value: statsData?.totalSales?.toString() || '0', change: '+8', icon: <TrendingUp className="w-5 h-5" />, color: 'purple' },
  ];

  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
  const revenueData = statsData?.revenueByMonth?.length > 0 
    ? statsData.revenueByMonth.map((d: any) => d.revenue)
    : [28500, 42000, 35600, 51200, 48900, 67800]; // Fallback demo data
