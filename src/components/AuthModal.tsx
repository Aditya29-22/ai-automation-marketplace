...
export default function AuthModal() {
  const { isAuthModalOpen, setAuthModalOpen, authMode, setAuthMode, loginDemo } = useStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      // Demo mode — use local auth
      loginDemo(name || email.split('@')[0], email);
      setLoading(false);
      return;
    }
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    const supabase = getSupabaseClient();
    if (!supabase) {
      loginDemo(provider + '-user', provider + '@demo.com');
      return;
    }
