...
export default function CheckoutPage() {
  const { cart, cartTotal, clearCart, isLoggedIn, user, setAuthModalOpen, addPurchase, isCheckoutComplete, setCheckoutComplete } = useStore();
  const { navigate: setCurrentPage } = useNavigation();
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderRes = await fetch('/api/payments/create-order', {
        ...
        body: JSON.stringify({
          items: cart.map(item => ({
            automation_id: item.product.id,
            price: item.product.price,
            with_maintenance: item.withMaintenance,
      ...
      const options = {
        ...
        name: 'AutomateStore',
        description: `Order: ${cart.length} automation${cart.length !== 1 ? 's' : ''}`,
        ...
        handler: async (response: any) => {
          // Step 4: Verify payment via our API
          try {
